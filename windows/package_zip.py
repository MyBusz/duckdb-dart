...
MEMBER_NAME = "duckdb.dll"
ARCHIVE_NAME = "duckdb-windows-x64.zip"
...
def main() -> None:
    ...
    if len(sys.argv) == 4 and sys.argv[1] == "--publish":
        ...
        if not stat.S_ISREG(staged_stat.st_mode) or staged.is_symlink():
            fail("staged archive must be a regular non-symlink file")
        validate_archive(staged)
        output_dir.mkdir(parents=True, exist_ok=True)
        os.replace(staged, output_dir / ARCHIVE_NAME)
        return
    if len(sys.argv) != 3:
        fail(
            f"usage: {sys.argv[0]} LIBRARY OUTPUT_DIRECTORY | "
            f"{sys.argv[0]} --publish STAGED_ARCHIVE OUTPUT_DIRECTORY"
        )
